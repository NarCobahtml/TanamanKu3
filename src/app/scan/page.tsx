...
const steps = [
  { n: 1, title: 'Buka Kamera', desc: 'Izinkan akses kamera dan arahkan ke daun yang ingin diperiksa.' },
  { n: 2, title: 'Ambil Foto', desc: 'Pastikan daun terlihat jelas di dalam bingkai sebelum mengambil foto.' },
  { n: 3, title: 'Lihat Hasil', desc: 'AI menganalisis foto dan menampilkan kemungkinan penyakit beserta penanganannya.' },
];
...
export default function ScanPage() {
  ...
  const analyze = async (dataUrl: string) => {
    ...
    try {
      ...
      setAnalyzing(false);
      setCameraError(err instanceof Error ? err.message : 'Analisis gagal, coba lagi.');
      void startCamera();
    }
  };
  ...
  return (
    ...
    <SitePage>
      ...
      <PageHeader
        ...
        accent="Tanaman"
        description="Ambil foto daun atau unggah gambar untuk menganalisis kemungkinan penyakit."
      />
      ...
      <div className="mx-auto grid w-full max-w-7xl gap-8 px-4 pt-12 sm:px-6 lg:grid-cols-[1fr_300px]">
        ...
        <div className="relative aspect-[4/3] overflow-hidden rounded-xl border border-border bg-secondary/60 transition-colors hover:border-primary/40 md:aspect-video">
          <video
            ...
            muted
            playsInline
            poster="/figma-assets/leaf-macro.jpg"
          ...
          {/* Scan line */}
          {cameraState === 'ready' && <div className="scanline" aria-hidden="true" />}
          ...
          {analyzing && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-secondary/85 backdrop-blur-sm">
              ...
              <p className="text-sm font-medium">Menganalisis daun...</p>
            </div>
          )}
        </div>
        ...
        <aside className="flex flex-col gap-6">
          <div className="rounded-xl border border-border bg-card p-5 transition-colors hover:border-primary/40 hover:bg-accent/30">
            <p className="overline">Kuota Scan</p>
            <p className="tnum mt-2 text-lg font-bold">3/5</p>
            <p className="text-xs text-muted-foreground">scan gratis tersisa</p>
            <div
            ...
            <p className="mt-2 text-xs text-muted-foreground">Reset setiap bulan</p>
          </div>
          ...
          <div className="space-y-2.5">
            ...
            <Button
              ...
              aria-label="Aktifkan atau matikan lampu kilat"
            >
              <Zap className="h-4 w-4" aria-hidden="true" />
              {flashOn ? 'Kilat Aktif' : 'Kilat'}
            </Button>
            <Button
      ...
      <TkReveal className="mx-auto w-full max-w-7xl px-4 pt-16 sm:px-6">
        <ol className="grid divide-y divide-border rounded-xl border border-border bg-card transition-colors hover:border-primary/40 md:grid-cols-3 md:divide-y-0 md:divide-x" aria-label="Cara kerja scan">
          {steps.map((step) => (
        ...
        <p className="mt-6 flex items-start gap-2 text-xs text-muted-foreground">
          ...
          Hasil analisis bersifat rekomendasi. Untuk diagnosis lanjutan konsultasikan dengan ahli.
        </p>
      </TkReveal>
      </div>
    </SitePage>
      ...
      <header className="absolute left-0 right-0 top-0 z-10 bg-black/45">
        <div className="mx-auto flex max-w-lg items-center justify-between px-4 py-4">
          <button onClick={() => window.history.back()} className="text-white" aria-label="Kembali">
            <ArrowLeft className="h-6 w-6" />
          </button>
          <h1 className="text-lg font-semibold">Scan Tanaman</h1>
          <button className="text-white" onClick={() => void startCamera()} aria-label="Muat ulang kamera">
      ...
      <main className="relative z-[1] flex min-h-screen items-center justify-center px-6">
        <div className="relative h-[min(68vh,480px)] w-[min(82vw,320px)]">
          ...
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-black/60 px-6 py-3 backdrop-blur-sm">
            ...
            {cameraState === 'error' && !analyzing && <button className="mt-2 block w-full text-xs font-semibold text-primary/10" onClick={() => void startCamera()}>Coba lagi</button>}
          </div>
        </div>
      </main>
      ...
      <div className="absolute bottom-0 left-0 right-0 z-10 bg-black/50 backdrop-blur-sm">
        <div className="mx-auto flex max-w-lg items-center justify-between px-8 py-8">
          ...
          <button onClick={capturePhoto} disabled={cameraState !== 'ready' || analyzing} className="flex h-20 w-20 items-center justify-center rounded-full border-4 border-white disabled:opacity-50" aria-label="Ambil foto">
          ...
          <button onClick={() => void toggleFlash()} className={`flex h-16 w-16 items-center justify-center rounded-full ${flashOn ? 'bg-primary/10/30' : 'bg-black/60'}`} aria-label="Flash">
            <Zap className={`h-6 w-6 ${flashOn ? 'text-primary/10' : 'text-white'}`} fill={flashOn ? 'currentColor' : 'none'} />
          </button>
        </div>
      </div>
        </div>
