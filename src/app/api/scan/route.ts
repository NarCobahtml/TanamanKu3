...
// Proxy Roboflow classification. Key tidak pernah sampai ke klien.
// multipart part WAJIB bernama 'file' (bukan 'image').
export async function POST(request: Request) {
  const key = process.env.ROBOFLOW_API_KEY;
  const modelId = process.env.ROBOFLOW_MODEL_ID;
  if (!key || !modelId) {
    return NextResponse.json({ error: 'Scan engine belum dikonfigurasi.' }, { status: 500 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: 'Gambar tidak ditemukan pada request.' }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: 'Tidak bisa menghubungi mesin scan. Coba lagi.' }, { status: 502 });
  }
}
